<div class="relative" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
    ...
    <!-- Hero Section: Initial visual impact for visitors -->
    <section class="min-h-screen flex items-center justify-center relative overflow-hidden px-6">
        ...
        <div class="max-w-5xl w-full grid md:grid-cols-2 gap-16 items-center reveal" :class="{ 'active': loaded }">
            ...
            <div class="order-2 md:order-1 text-center md:text-left space-y-6">
                <span class="text-blue-500 font-black text-xs uppercase tracking-[0.3em]">Available for work</span>
                ...
                <h3
                    class="text-xl font-bold text-slate-600 dark:text-slate-300 tracking-tight transition-colors duration-500">
                    {{ $owner->profile->job_title }}
                </h3>

                <p class="text-slate-500 dark:text-slate-400 leading-relaxed max-w-md transition-colors duration-500">
                    {{ $owner->profile->bio }}
                </p>
                ...
                <div class="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                    ...
                    <a href="#contact"
                        class="px-8 py-4 bg-white/10 glass text-slate-900 dark:text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200/50 transition-all">
                        Contact Me
                    </a>
            ...
            <div class="order-1 md:order-2 flex justify-center relative">
                <div class="relative group">
                    ...
                    <img src="{{ asset('storage/' . $owner->profile->avatar) }}" alt="{{ $owner->name }}"
                        class="relative rounded-full w-64 h-64 md:w-80 md:h-80 object-cover grayscale hover:grayscale-0 transition-all duration-700 border-4 border-white dark:border-slate-800 shadow-2xl" />
                </div>
            </div>
        </div>
    </section>

    <!-- Main Content Area: Intersection observed sections -->
