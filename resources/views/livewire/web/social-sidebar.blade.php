    ...
    <div class="glass p-3 rounded-full flex flex-col gap-5 shadow-2xl">
        @foreach ($owner->profile->socialLinks as $socialLink)
            <a href="{{ $socialLink->url }}" target="_blank"
                class="w-11 h-11 flex items-center justify-center rounded-full bg-slate-200/50 dark:bg-white/5 text-slate-700 dark:text-slate-400 hover:bg-blue-600 hover:text-white transition-all duration-300 shadow-sm">
